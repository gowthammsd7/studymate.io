...
export default function ToolTabs() {
  ...
  return (
    <section id="tools" className="mx-auto max-w-7xl px-4 pb-16 pt-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-3xl text-center">
        <h2 className="text-3xl font-extrabold tracking-tight sm:text-4xl">Hands-on tools</h2>
        <p className="mt-3 text-muted-foreground">Paste notes or add tasks — everything runs locally.</p>
      </div>
        ...
        {tabs.map((t) => (
          <button
            ...
            className={cn(
              ...
              active === t.id ? "bg-primary text-primary-foreground" : "hover:bg-accent hover:text-accent-foreground",
            )}
          >
            {t.label}
          </button>
...
function QuizGenerator() {
  ...
  function makeMCQs(input: string, n: number): MCQ[] {
    ...
    const topTerms = [...freq.entries()].sort((a, b) => b[1] - a[1]).map(([w]) => w).slice(0, 50);
    ...
    for (const s of candidates) {
      ...
      const blanked = s.replace(new RegExp(`\\b${answerWord}\\b`, "i"), "_____");
      const distractors = shuffle(topTerms.filter((w) => w !== answerWord)).slice(0, 3);
      const options = shuffle([answerWord, ...distractors]).map((o) => capitalize(o));
      const answer = options.findIndex((o) => o.toLowerCase() === answerWord.toLowerCase());
      ...
      <div className="glass">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <input type="number" min={1} max={15} value={count} onChange={(e) => setCount(parseInt(e.target.value || "5"))} className="w-20 rounded-md border bg-background px-2 py-1" />
        ...
        {quiz.length > 0 && (
          <div className="mt-6 space-y-6">
            {quiz.map((q, i) => (
              <div key={i} className="rounded-lg border p-4">
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  {q.options.map((opt, j) => (
                    <label key={j} className={cn("flex cursor-pointer items-center gap-2 rounded-md border p-2", answers[i] === j ? "border-violet-600 bg-violet-50" : "hover:bg-accent") }>
                      <input type="radio" name={`q-${i}`} checked={answers[i] === j} onChange={() => setAnswers((a) => a.map((x, idx) => (idx === i ? j : x)))} />
            ...
            <div className="flex items-center justify-between">
              ...
              {score !== null && (
                <div className="text-sm font-semibold">Score: {score}/{quiz.length}</div>
              )}
            </div>
          </div>
        )}
      </div>
      <Tips title="Tips for better quizzes" items={["Use 2–6 short paragraphs.", "Include key terms and definitions.", "Remove headings and lists for best results."]} />
    </>
  );
}
...
function FlashcardMaker() {
  ...
  function buildCards(input: string) {
    ...
    for (const s of sents) {
      ...
      if (parts.length >= 2) {
        out.push({ front: capitalize(parts[0]), back: parts.slice(1).join(" — ") });
      } else {
        const ws = words(s);
        if (ws.length > 6) {
          out.push({ front: capitalize(ws.slice(0, 5).join(" ")) + "?", back: capitalize(ws.slice(5).join(" ")) });
        }
      }
        ...
        {current && (
          <div className="mt-6">
            ...
            <div className="mt-4 flex items-center justify-between">
              <Button variant="outline" onClick={() => setI((x) => Math.max(0, x - 1))}>Prev</Button>
              <div className="text-sm text-muted-foreground">{i + 1} / {cards.length}</div>
              <Button onClick={() => setI((x) => Math.min(cards.length - 1, x + 1))}>Next</Button>
            </div>
          </div>
        )}
      </div>
      <Tips title="Better flashcards" items={["Use 'Term - Definition' style lines.", "Keep terms short; details go on back.", "Aim for 10–20 cards per topic."]} />
    </>
  );
}
...
function StudyPlanner() {
  type Task = { id: string; title: string; minutes: number; due?: string; done?: boolean };
  ...
  function add() {
    ...
    setTasks((t) => [{ id: cryptoRandom(), title: title.trim(), minutes: Math.max(5, minutes), due: due || undefined }, ...t]);
    setTitle("");
  }
    ...
    <>
      <div className="glass">
        ...
        <div className="grid gap-3 sm:grid-cols-4">
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Task title" className="sm:col-span-2 rounded-md border bg-background px-3 py-2 text-sm" />
          <input type="number" min={5} value={minutes} onChange={(e) => setMinutes(parseInt(e.target.value || "25"))} className="rounded-md border bg-background px-3 py-2 text-sm" placeholder="Minutes" />
        ...
        <ul className="mt-4 divide-y rounded-lg border">
          {tasks.length === 0 && (
            <li className="p-4 text-sm text-muted-foreground">No tasks yet. Add your first study task.</li>
          )}
          {tasks.map((t) => (
            <li key={t.id} className="flex items-center gap-3 p-3">
              <input type="checkbox" checked={!!t.done} onChange={() => toggle(t.id)} />
              <div className="flex-1">
                <div className={cn("text-sm font-medium", t.done && "line-through text-muted-foreground")}>{t.title}</div>
                <div className="text-xs text-muted-foreground">{t.minutes} min {t.due ? `• due ${t.due}` : ""}</div>
              </div>
              <Button variant="outline" onClick={() => remove(t.id)}>Delete</Button>
            </li>
          ))}
        </ul>
      </div>
      <Tips title="Planning advice" items={["Break big topics into 25–50 min blocks.", "Add due dates to keep momentum.", "Mark done to track progress."]} />
    </>
  );
}
...
function Pomodoro({ defaultMinutes = 25 }: { defaultMinutes?: number }) {
  ...
  return (
    <div className="inline-flex items-center gap-2 rounded-full border px-3 py-1 text-sm">
      ...
      <Button size="sm" variant="outline" onClick={() => { setRunning(false); setSeconds(defaultMinutes * 60); }}>Reset</Button>
    </div>
  );
}
...
function Summarizer() {
  ...
  return (
    <>
      <div className="glass">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <input type="range" min="0.15" max="0.7" step="0.05" value={ratio} onChange={(e) => setRatio(parseFloat(e.target.value))} />
          </div>
        </div>
        <textarea
          value={text}
      ...
      <Tips title="Get sharper summaries" items={["Remove redundant headings.", "Prefer paragraphs over bullet lists.", "Adjust the length slider as needed."]} />
    </>
  );
}
